...
async function startBot() {
    try {
        ...
        // NOTE: use the `sockInstance` passed by the event listener, NOT the
        // outer `sock` variable. On reconnect, whatsappClient.js creates a
        // brand new socket internally, but the outer `sock` here would still
        // point at the old, dead connection — causing replies/posts to
        // silently fail after any disconnect. Always use the fresh instance.
        const sock = await connectToWhatsApp(
            async (m, sockInstance) => await handleIncomingMessage(m, sockInstance, config),
            { usePairingCode: true }
        );
